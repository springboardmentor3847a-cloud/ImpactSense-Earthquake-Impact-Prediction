...
USGS Earthquake API: Fetches recent (2023) earthquake data.

Historical CSV: A large local dataset (database.csv) provides a deep historical context.
...
A ColumnTransformer is created to automatically apply StandardScaler to numeric features and OneHotEncoder to categorical features (soil_type).

This "preprocessor" is bundled with the best-tuned XGBoost model into a single scikit-learn Pipeline object.

This pipeline, along with the LabelEncoder (for turning "Strong" into 2), is saved to disk using joblib (earthquake_pipeline.joblib).
...
The app runs the prediction using the loaded pipeline.predict() method.

It uses reverse_geocoder to find the country for the given latitude and longitude. Note: This may take a moment on the first run as it loads its local geo-database.

A Python dictionary (EMERGENCY_NUMBERS) maps the country code (e.g., "IN") to its specific helplines (e.g., "112").

The entire UI dynamically updates to show the color-coded result and the four-tab response section, now populated with both generic safety info and the specific, geo-located emergency numbers.
...
git clone https://github.com/your-username/earthquake-classifier.git
...
Run the training script (Milestone_2.py) to fetch data, process it, train the model, and save the .joblib files.
